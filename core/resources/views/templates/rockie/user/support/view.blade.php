                            ...
                            <div class="card-header card-header-bg d-flex flex-wrap justify-content-between align-items-center">
                                ...
                                <div class="d-flex">
                                    @if ($myTicket->status != Status::TICKET_CLOSE && $myTicket->user)
                                        <button class="btn btn-outline-danger close-button btn-sm confirmationBtn mr-2" type="button"
                                    ...
                                    @endif
                                    @auth
                                        <a href="{{ route('ticket.index') }}" class="btn btn-outline-primary close-button btn-sm ms-2">
                            ...
                            <div class="card-body  {{ !auth()->user() ? 'bg--body' : '' }}">
                                <form method="post" action="{{ route('ticket.reply', $myTicket->id) }}"
                                    ...
                                    <div class="text-end mb-3">
                                        <a href="javascript:void(0)" class="text-dark addFile"><i class="os-icon os-icon-paperclip"></i>
                                            <span class="attachment-text">@lang('Attach Files')</span></a>
                                    </div>
                                    ...
                                    <div class="form-group attachment-wrapper d-none">
                                        ...
                                        <p class="ticket-attachments-message text-muted mt-2">
                                            <span class="d-block"> @lang('Allowed File Extensions'): .@lang('jpg'), .@lang('jpeg'),
                                    ...
                                    <div class="form-group">
                                        <button type="submit" class="btn btn-primary w-100"> <i class="os-icon os-icon-reply"></i>
                                            @lang('Reply')</button>
                                    </div>
                                </form>
                            </div>
                                                    ...
                                                    <div class="mt-2">
                                                        @foreach ($message->attachments as $k => $image)
                                                            <a href="{{ route('ticket.download', encrypt($image->id)) }}"
                                                                class="me-3">
                                                                <i class="os-icon os-icon-file"></i> @lang('Attachment') {{ ++$k }}
                                                            </a>
                                                        @endforeach
                                                    </div>
                                                @endif
